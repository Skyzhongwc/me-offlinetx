...
func RegisterDenoms() {
	err := sdk.RegisterDenom("mec", sdk.OneDec())
	if err != nil {
		panic(err)
	}

	err = sdk.RegisterDenom("umec", sdk.NewDecWithPrec(1, 6))
	if err != nil {
		panic(err)
	}
}
...
func NewClient(url26657 string, url9090 string) (Client, error) {
	...
	if cli.Conn, err = grpc.Dial(url9090, grpc.WithInsecure()); err != nil {
		return cli, err
	}
	...
	cli.AuthClient = authtypes.NewQueryClient(cli.Conn)
